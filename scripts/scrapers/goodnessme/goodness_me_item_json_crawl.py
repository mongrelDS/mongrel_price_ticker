...
def get_brand_links_from_db(sample_fraction_divider: int = 20) -> List[str]:
    """Read brand links from MySQL, filter for goodnessme.ca, and sample rows.
    ...
    # Filter to goodnessme.ca only
    df_links = df_links[df_links['brand_url'].astype(str).str.contains(DOMAIN, case=False, na=False)]
    if df_links.empty:
        print("[bold yellow]⚠️ No goodnessme.ca links found in brand_link_list[/bold yellow]")
        return []
...
def get_collection_handle_from_link(url: str) -> Optional[str]:
    try:
        # Expected formats:
        # https://goodnessme.ca/collections/<handle>
        # https://goodnessme.ca/collections/<handle>?param=...
...
def fetch_root_products(session: requests.Session, base_url: str, limit: int = 100) -> List[dict]:
    """Fetch products from root Shopify endpoints as a fallback."""
    products: List[dict] = []
    try:
        ...
        pass
    # Try collections/all as well
    try:
        if len(products) < limit:
            url = f"{base_url}/collections/all/products.json"
            params = {"limit": str(max(1, min(limit - len(products), 250)))}
            r = session.get(url, params=params, headers={"Accept": "application/json"}, timeout=30)
            r.raise_for_status()
            data = r.json() or {}
            prods = data.get("products") or []
...
def main():
    ...
    if args.discover:
        ...
        print("[bold cyan]🚀 Starting goodnessme.ca item update from JSON...[/bold cyan]")
    ...
    base_url = "https://www.goodnessme.ca"
    ...
    if args.discover:
        ...
        # Probe root-level Shopify endpoints for products
        print("\nProbing root product endpoints...")
        root_eps = [
        ...
        for ep in root_eps:
            try:
                r = session.get(ep, headers={"Accept": "application/json"}, timeout=30)
                data = r.json()
                if isinstance(data, dict) and isinstance(data.get("products"), list) and data["products"]:
                    print(f"Root endpoint OK: {ep}")
                    p0 = data["products"][0]
                    if isinstance(p0, dict):
                        for k in list(p0.keys())[:50]:
                            product_field_keys_counter[k] += 1
                    ...
                    print(f"Root endpoint no products: {ep}")
            except Exception:
                print(f"Root endpoint failed: {ep}")
    ...
    for link in brand_links:
        # Probe JSON availability first; skip if none
        try:
            has_json, _json_eps = probe_json_endpoints(session, link)
            if not has_json:
                continue
        except Exception:
            pass
    ...
    # If none from collections, fall back to root products JSON
    if not products:
        try:
            root_prods = fetch_root_products(session, base_url, limit=100)
            for item in root_prods:
                try:
                    title = item.get('title', '')
                    vendor = item.get('vendor')
                    handle_p = item.get('handle') or ''
                    ...
                    # Prefer product.js for reliable variants (cents vs dollars)
                    pjs = fetch_product_js(session, base_url, handle_p) if handle_p else None
                    variants = (pjs or {}).get('variants') if isinstance(pjs, dict) else None
                    if not variants:
                        variants = item.get('variants') or []
    ...
    print("[bold green]✅ Completed goodnessme.ca item update[/bold green]")
