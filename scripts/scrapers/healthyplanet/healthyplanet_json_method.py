...
def main():
    parser = argparse.ArgumentParser(description="Healthy Planet scraper")
    ...
    start_ts = time.time()
    print("[bold cyan]🚀 Starting scraper for healthyplanetcanada.com...[/bold cyan]")
    ...
    graphql_url = "https://www.healthyplanetcanada.com/graphql"
    ...
    # GraphQL query with extra fields
    graphql_query = (
        """
    ...
    for idx, query in enumerate(search_queries, start=1):
        ...
        try:
            ...
            brand_map = build_brand_mapping(prod_data.get("aggregations"))
            for item in prod_data.get("items", []) or []:
                try:
                    ...
                    link = f"https://www.healthyplanetcanada.com/{url_key}{url_suffix}" if url_key else ''
                    ...
                    brand_name = brand_map.get(str(brand_id)) if brand_id is not None else None
                    ...
                    # Fetch PDP JSON-LD to try to get barcode (gtin)
                    barcode = None
                    if link:
                        try:
    ...
    print("[bold green]✅ Completed Healthy Planet item update[/bold green]")
