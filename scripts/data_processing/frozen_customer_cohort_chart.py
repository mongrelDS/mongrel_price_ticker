...
SHEET_REVENUE = 'Newfrozen_Revenue'
SHEET_ORDERS = 'Newfrozen_Order_Count'
SHEET_CUSTOMERS = 'Newfrozen_Customer_Count'
WRITE_CELL = 'A6'
...
def trim_by_min_cohort_week(p: pd.DataFrame, min_label: str) -> pd.DataFrame:
    """Keep only rows with Cohort Week >= min_label (e.g., '25 Wk 18')."""
    if 'Cohort Week' not in p.columns:
        return p
    min_year, min_week = _parse_label_year_week(min_label)
    if min_year is None or min_week is None:
        return p
    parsed = p['Cohort Week'].apply(_parse_label_year_week)
    years = parsed.apply(lambda t: t[0])
    weeks = parsed.apply(lambda t: t[1])
    mask = (years > min_year) | ((years == min_year) & (weeks >= min_week))
    return p.loc[mask].copy()
...
def build_pivots_from_sources(engine) -> dict[str, pd.DataFrame]:
    ...
    # Filter to only orders tagged as frozen (case-insensitive)
    if 'order_tags' in ords.columns:
        mask_frozen = ords['order_tags'].astype(str).str.contains('frozen', case=False, na=False)
        ords = ords.loc[mask_frozen].copy()
    ...
    # Restrict to orders on/after 2025-05-05
    start_date = pd.Timestamp('2025-05-05')
    ords = ords.loc[ords['order_date'] >= start_date].copy()
...
def main():
    ...
    # Trim to cohorts not older than '25 Wk 18'
    min_label = '25 Wk 18'
    if 'revenue' in pivots:
        print(f"Writing revenue -> {SHEET_REVENUE}")
        write_df_to_sheet(finalize_display_format(trim_by_min_cohort_week(pivots['revenue'], min_label)), SPREADSHEET_ID, SHEET_REVENUE, WRITE_CELL)

    print(f"Writing orders -> {SHEET_ORDERS}")
    write_df_to_sheet(finalize_display_format(trim_by_min_cohort_week(pivots['orders'], min_label)), SPREADSHEET_ID, SHEET_ORDERS, WRITE_CELL)

    print(f"Writing customers -> {SHEET_CUSTOMERS}")
    write_df_to_sheet(finalize_display_format(trim_by_min_cohort_week(pivots['customers'], min_label)), SPREADSHEET_ID, SHEET_CUSTOMERS, WRITE_CELL)
