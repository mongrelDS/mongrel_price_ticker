#!/usr/bin/env python3
"""
Test IPBurger proxy with correct credentials from dashboard
"""
...
def test_correct_ipburger():
    """Test IPBurger with correct credentials from dashboard"""
    print("🔍 Testing IPBurger with correct credentials...")
    ...
    print(f"🏙️ Location: Toronto, Canada")
    print(f"⏱️ Sticky IP: 30 Minutes")
...
def test_healthyplanet_with_proxy():
    """Test Healthy Planet specifically with working proxy"""
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if hp_response.status_code == 200:
                    ...
                    if "healthyplanet" in hp_response.text.lower():
                    ...
                    print(f"    📄 Response: {hp_response.text[:200]}")
            else:
                print(f"    ❌ Basic connection failed: {response.text[:100]}")
...
def main():
    """Main test function"""
    print("🚀 IPBurger Proxy Test with Correct Credentials")
    print("=" * 60)
    ...
    print("="*60)
    config, test_url = test_correct_ipburger()
    ...
    # Test 2: Healthy Planet specific
    print("\n" + "="*60)
    print("TEST 2: Healthy Planet Specific Test")
    print("="*60)
    hp_config = test_healthyplanet_with_proxy()
    
    if hp_config:
        print(f"\n✅ Healthy Planet test successful!")
        print(f"Working config: {hp_config['name']}")
    else:
        print(f"\n❌ Healthy Planet test failed")
    ...
    if config or hp_config:
        print("✅ IPBurger proxy is working!")
        print("Ready to create Healthy Planet scraper")
        return True
    else:
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\n⚠️ Proxy needs further investigation")
